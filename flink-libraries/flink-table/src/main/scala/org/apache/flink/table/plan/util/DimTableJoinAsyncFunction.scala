...
class DimTableJoinAsyncFunction (
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    val tableSources = ServiceLoader.load(classOf[DimTableSourceProvider])
    tableSource =
      tableSources.asScala.filter(_.typeName().equalsIgnoreCase(tableType)).toList match {
        case Nil =>
          sys.error(s"DimTableSourceProvider for type [$tableType] not found.")
        case source :: Nil =>
          ...
          source
        case sources =>
          sys.error(s"Multiple DimTableSourceProviders for type [$tableType] encountered: " +
    ...
    ThreadPool.submit(new Runnable {
      override def run(): Unit = {
        try {
          ...
          val rightOutputIter = dimOutput(leftKeyValues)
          ...
          case t:Throwable =>
            LOG.error("Async invocation error.", t)
      ...
      def dimOutput(leftKeyValues: Array[AnyRef]): java.util.Iterator[Row] = {
